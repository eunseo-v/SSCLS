...
train_pipeline = [
    ...
    dict(type='GenerateTaiChiPoseTarget', with_kp=False, with_limb=True), # 这个函数中的left_kp指的是5通道中的left和right，所以值不同
...
val_pipeline = [
    ...
    dict(type='GenerateTaiChiPoseTarget', with_kp=False, with_limb=True),
    dict(type='FormatShape', input_format='NCTHW_Heatmap'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
...
test_pipeline = [
    ...
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(64, 64), keep_ratio=False),
    dict(type='GenerateTaiChiPoseTarget', with_kp=False, with_limb=True, double=True), 
    dict(type='FormatShape', input_format='NCTHW_Heatmap'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs'])
]
...
'''
output_config = dict(
    out = './model_pth/exp4/t9nsnr_wfwc_5p_lb/test_result/results.pkl'
) # 保存测试集的各类分类概率和样本名 keys:'outputs','sample_names'
eval_config = dict(
    metric_out = './model_pth/exp4/t9nsnr_wfwc_5p_lb/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix', 't_sne_vis'],
) # train中不写test_last test_best 在test.py中测试
work_dir = './model_pth/exp4/t9nsnr_wfwc_5p_lb'
